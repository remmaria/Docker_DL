#!/usr/bin/env python3
"""
Etapa 5b: aplica uma RRIN3D treinada (checkpoint da etapa 4b) aos sujeitos
de um split (tipicamente "test"), reconstruindo as direcoes-alvo do
esquema de trincas (scripts/02b_build_rrin_triplets.py) volume inteiro
(blocos/patches com overlap, costurados por media -- mesmo esquema de
scripts/05_reconstruct_rcae.py).

Reconstroi TODOS os alvos presentes no esquema de trincas para este
(shell,n_level), MESMO os marcados `valid=False` (residuo de colinearidade
acima do teto) -- de proposito: pra comparar com baseline_sh/RCAE na MESMA
cobertura de direcoes held-out, e pra que 06_evaluate_reconstruction.py
possa depois estratificar o erro por `valid` (ver coluna extra
`rrin_valid.npy` gravada ao lado da reconstrucao) e mostrar exatamente
onde/quanto a suposicao de fluxo falha (ver protocolo, secao 10.1).

Uso:
    python scripts/05b_reconstruct_rrin.py \
        --manifest work_dir/manifest.csv \
        --triplets-dir work_dir/subsampling \
        --checkpoint work_dir/rrin_checkpoints/shell1000_n10/best.pt \
        --shell-b 1000 --n-level 10 \
        --out-dir work_dir/rrin_recon \
...
def main():
    ...
    for e in entries:
        ...
        # bvec/t de cada alvo, indexados por k (0..n_target-1) -- constantes
        # ao longo dos patches, calculados uma vez fora do loop espacial.
        ...
        quality_all = None
        if use_quality_cond:
            # mesma normalizacao (/90) de utils/rrin_dataset.py -- checkpoint
            # treinado com use_quality_cond=True exige esse tensor no forward.
        ...
        with torch.no_grad():
            for (ox, oy, oz) in origins:
                ...
                # -> (n_target, 1, ps, ps, ps), um "batch" = as n_target trincas
        ...
        # colunas extras (nao lidas por 06_evaluate_reconstruction.py hoje,
        # mas na mesma ordem de target_idx -- uso pra estratificar o erro
        # por qualidade geometrica da trinca depois, ver protocolo 10.1)
        ...
        print(f"{tag}: reconstrucao RRIN salva em {sub_out} "
