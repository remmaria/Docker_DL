#!/usr/bin/env python3
"""
Etapa 5d: aplica uma AMT3D treinada (checkpoint da etapa 4c, ver
model/amt3d.py e scripts/04c_train_amt.py) aos sujeitos de um split
...
scripts/05b_reconstruct_rrin.py -- unica coisa que muda de verdade e qual
modelo e carregado (model.amt3d.build_amt_model em vez de
model.rrin3d.build_rrin_model) e quais chaves de arquitetura sao lidas do
checkpoint (num_fields, corr_radius em vez de num_layers).
...
scripts/05b_reconstruct_rrin.py (comparar na mesma cobertura de direcoes
held-out, e permitir a scripts/06_evaluate_reconstruction.py estratificar
o erro por validade geometrica da trinca).

IMPORTANTE sobre o nome do arquivo de validade (`rrin_valid.npy`):
scripts/06_evaluate_reconstruction.py detecta a coluna "triplet_valid" por
um caminho HARDCODED (`sub_dir / "rrin_valid.npy"`, ver o codigo-fonte la
antes de decidir isso), nao por um nome generico configuravel. Como o
conceito de trinca valida/invalida usado aqui e EXATAMENTE o mesmo do RRIN
(vem do MESMO esquema de trincas, scripts/02b_build_rrin_triplets.py, via o
MESMO RRINTripletDataset) -- nao um conceito novo do AMT3D -- a escolha
correta e REUSAR o mesmo nome de arquivo (nao inventar "amt_valid.npy"),
para que 06_evaluate_reconstruction.py estratifique aggregate_valid/
aggregate_invalid para AMT3D sem nenhuma modificacao no script de
avaliacao. O nome "rrin_valid.npy" ficou "historico" (cunhado quando so
existia a linha RRIN), mas o conteudo que ele representa (validade da
trinca) e uma propriedade do ESQUEMA DE TRINCAS, nao do modelo que o
consome -- por isso reusar o nome é a opção mais consistente, e não uma
gambiarra.
...
Uso:
    python scripts/05d_reconstruct_amt.py \
        ...
        --checkpoint work_dir/amt_checkpoints/shell1000_n10/best.pt \
        --shell-b 1000 --n-level 10 \
        --out-dir work_dir/amt_recon \
...
def main():
    ...
    model = build_amt_model(base_ch=ckpt_args.get("base_ch", 16),
                             max_disp=ckpt_args.get("max_disp", 0.5),
                             num_fields=num_fields, corr_radius=corr_radius,
    ...
    print(f"Checkpoint carregado (epoca {ckpt.get('epoch')}, val_loss {ckpt.get('val_loss')}, "
          f"num_fields={num_fields}, corr_radius={corr_radius}, norm_type={norm_type})")
    ...
    for e in entries:
        ...
        # (rrin_valid.npy, nao "amt_valid.npy") -- ver docstring do modulo,
        # scripts/06_evaluate_reconstruction.py detecta esse nome de forma
        # hardcoded para estratificar aggregate_valid/aggregate_invalid.
        ...
        print(f"{tag}: reconstrucao AMT3D salva em {sub_out} "
