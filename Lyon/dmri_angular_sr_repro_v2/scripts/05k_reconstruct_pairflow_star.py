#!/usr/bin/env python3
"""
Etapa 5k: aplica uma PairFlowStar treinada (checkpoint da etapa 4i, ver
scripts/04i_train_pairflow_star.py e model/pairflow_star.py) aos sujeitos de
um split (tipicamente "test"), reconstruindo as direcoes-alvo do esquema de
trincas volume inteiro (blocos/patches com overlap, costurados por media --
mesmo esquema de scripts/05f_reconstruct_rrin_star.py).

Espelha DELIBERADAMENTE scripts/05f_reconstruct_rrin_star.py (mesma leitura
do feixe `{key}__ens_*`, mesma reconstrucao por janela deslizante) --
DIFERENCA: nao ha `use_quality_cond` aqui (so' `weight_quality_cond`, ver
model/pairflow_star.py), e o M usado tambem e lido do proprio checkpoint
(`ckpt["args"]["ensemble_m"]`).

Uso:
    python scripts/05k_reconstruct_pairflow_star.py \
        --manifest work_dir/manifest.csv \
        --triplets-dir work_dir/subsampling \
        --checkpoint work_dir/pairflow_star_checkpoints/shell1000_n16_pfstar3/best.pt \
        --shell-b 1000 --n-level 16 \
        --out-dir work_dir/pairflow_star_recon \
...
def sliding_window_origins(shape, patch_size, stride):
    """Identico a scripts/05f_reconstruct_rrin_star.py -- duplicado aqui de
    proposito (convencao do projeto: scripts numerados nao se importam entre
    si)."""
    origins = []
...
def main():
    ...
    need_quality = weight_quality_cond
    norm_type = ckpt_args.get("norm_type", "instance")
    freeze_flow = ckpt_args.get("freeze_flow", False)
    ...
    print(f"Reconstruindo {len(entries)} sujeito(s) (PairFlowStar, M={ensemble_m}): "
    ...
    for e in entries:
        ...
        # bvec_t_all e' construido so' por compatibilidade de assinatura com
        # PairFlowStar.forward -- ver docstring do modulo, NAO e' usado pelo
        # modelo (a linha PairFlow nao condiciona o fluxo pelo alvo).
        ...
        np.save(sub_out / "pairflow_valid.npy", valid)
        np.save(sub_out / "pairflow_residual_deg.npy", residual_deg)
        np.save(sub_out / "pairflow_gap_deg.npy", gap_deg)
        np.save(sub_out / "pairflow_star_n_pares.npy", ens_valid.sum(axis=1))
        mask_path = shell_out / "mask.npy"
        if not mask_path.exists():
            np.save(mask_path, mask)
        n_invalid = int((~valid).sum())
        print(f"{tag}: reconstrucao PairFlowStar (M={ensemble_m}) salva em {sub_out} "
