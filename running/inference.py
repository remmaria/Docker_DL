...
def get_neighbors_info(
    ...
    """
    Busca vizinhos angulares consistentes com o treino.
    """
    ...
    # remove b0
    idx_input = idx_input[
        all_bvals[idx_input] > 50
    ]

    # remove target
    idx_input = idx_input[
        idx_input != target_global_idx
    ]
    ...
    if len(idx_input) == 0:
        ...
        return (
            [target_global_idx] * k_neighbors,
            np.zeros((k_neighbors, 4), dtype=np.float32)
        )

    # hemisfério alinhado
    pool_vecs_hemi = np.array([
        v if np.dot(v, target_v) >= 0 else -v
        for v in all_bvecs[idx_input]
    ])
    ...
    if len(neighbor_indices) < k_neighbors:
        ...
        restantes = idx_input[restantes_mask]
    ...
    return (
        neighbor_indices,
        np.array(n_coords, dtype=np.float32)
    )
...
def Quick_DTI(path_nii, path_bval, path_bvec, out_prefix, inf_folder):
    ...
    gtab  = gradient_table(path_bval, bvecs=path_bvec)
...
checkpoint_path = "/ix1/tibrahim/rmm270/Docker_DL/1669118/checkpoints/1758821/code_backup_new/checkpoints/1866817/model_best.pt"
K               = 8
alpha           = 2.0
bmax            = 2000.0
...
for i in range(len(bvals)):
    ...
    if i in neighbor_idx_in_full:
        print(f"TARGET LEAK! direção {i} entrou nos vizinhos")
